...
import numpy as np # Para el cálculo del RSI real
...
# --- INICIALIZACIÓN (MANTENIENDO TODO) ---
...
with st.sidebar:
    ...
    dist_recompra = 0.5 / 100 # Distancia para abrir niveles si baja (0.5%)
...
if bot_on:
    try:
        # 1. Obtener Precio y calcular RSI Real
        ...
        # Calculamos el RSI con los datos reales acumulados
        rsi_real = calcular_rsi(st.session_state.precios_hist)

        # 2. ENTRADA INICIAL (Nivel 1)
        ...
        # 3. LÓGICA DE RECOMPRA (Si baja de la línea de Nivel 1)
        else:
            ultima_pos = st.session_state.posiciones[-1]
            # Si el precio actual es menor al de la última compra por la distancia fijada
            if precio <= ultima_pos['precio'] * (1 - dist_recompra):
                ...
                # Log de Recompra
                new_log = pd.DataFrame([{"Hora": datetime.now().strftime("%H:%M:%S"), "Evento": f"RECOMPRA N{nuevo_id}", "Precio": precio, "PNL": "---"}])
                st.session_state.log_df = pd.concat([new_log, st.session_state.log_df]).reset_index(drop=True)

        # 4. LÓGICA DE CIERRE (Profit o RSI 70)
        for i, pos in enumerate(st.session_state.posiciones):
            target = pos['precio'] * (1 + dist_grid)
            if (precio >= target or rsi_real >= 70) and precio > pos['precio']:
        ...
        # --- PANEL DE MÉTRICAS (MANTENIDO) ---
        c1, c2, c3 = st.columns(3)
        label_billetera = "SALDO REAL" if es_real else "SALDO VIRTUAL"
        ...
        with c3: st.markdown(f"<div class='metric-card'><div class='metric-label'>RSI REAL</div><div class='metric-value' style='color:#00FFAA;'>{rsi_real:.1f}</div></div>", unsafe_allow_html=True)

        # --- GRÁFICO (MANTENIDO CON LÍNEAS) ---
        fig = go.Figure()
    ...
    st.info("👋 Bot en pausa. Revisa el menú lateral para configurar.")

...
def conectar_pionex(api_key, secret_key):
    if not api_key or not secret_key:
        st.error("⚠️ Las claves API y Secret no pueden estar vacías.")
        return None
    try:
        ...
        if not markets or not isinstance(markets, dict):
            ...
            return None
        st.success("✅ Conexión exitosa a Pionex.")
        return exchange
    except ccxt.AuthenticationError:
        st.error("❌ Error de autenticación con Pionex. Verificá tus claves.")
        return None

...
if not st.session_state.autenticado:
    ...
    c_h1.markdown(f"## ☀️ H y G Inovaciones - 👤 {st.session_state.user_name}")
        ...
        try:
            ...
            if not st.session_state.ordenes_malla and rsi_val <= rsi_compra:
                ...
                for i in range(niveles):
                    st.session_state.ordenes_malla.append({'id': i+1, 'precio': round(precio_act * (1 - (i * distancia)), 4), 'monto': round(monto_n, 2), 'estado': 'PENDIENTE'})
            ...
            # --- LÓGICA DE CIERRE INDEPENDIENTE (AQUÍ ESTÁ EL CAMBIO) ---
            if st.session_state.posiciones:
                p_prom = sum(p['entrada'] for p in st.session_state.posiciones) / len(st.session_state.posiciones)
                p_tp = p_prom * (1 + tp_global)
                
                # VENDE SI: El precio toca el Profit O el RSI toca la Venta (Independientes)
                if precio_act >= p_tp or rsi_val >= rsi_venta:
                    total_inv = sum(p['monto'] for p in st.session_state.posiciones)
                    ganancia = (total_inv * (precio_act / p_prom - 1)) * lev
                    
                    st.session_state.historial_pnl.append({'Fecha': datetime.now().strftime("%H:%M"), 'Par': par, 'Ganancia': round(ganancia, 2)})
            ...
            if st.session_state.posiciones: fig.add_hline(y=p_tp, line_color="#00FFFF", annotation_text="GANANCIA")
            st.plotly_chart(fig, use_container_width=True)

            col_a, col_b = st.columns(2)
            with col_a: st.subheader("📋 Malla"); st.table(st.session_state.ordenes_malla)
            with col_b: st.subheader("📜 Historial"); st.table(st.session_state.historial_pnl)

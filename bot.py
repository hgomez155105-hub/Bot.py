import streamlit as st
import pandas as pd
import requests
import time
import plotly.graph_objects as go
import numpy as np
from datetime import datetime

# --- CONFIGURACIÓN DE PÁGINA Y LIMPIEZA DE INTERFAZ ---
st.set_page_config(page_title="H y G Inovaciones", layout="wide", page_icon="☀️")

st.markdown("""
    <style>
    #MainMenu {visibility: hidden;}
    footer {visibility: hidden;}
    header {visibility: hidden;}
    .stApp { background-color: #0B0E11 !important; margin-top: -60px; }
    [data-testid="stMetricValue"] { color: #F0B90B !important; font-size: 1.6rem !important; }
    .user-tag { background: #1E2329; padding: 5px 15px; border-radius: 20px; border: 1px solid #F0B90B; color: white; }
    </style>
    """, unsafe_allow_html=True)

# --- FUNCIONES TÉCNICAS ---
def obtener_top_20_binance():
    try:
        url = "https://api.binance.com/api/v3/ticker/24hr"
        res = requests.get(url).json()
        df_vol = pd.DataFrame(res)
        df_vol = df_vol[df_vol['symbol'].str.endswith('USDT')]
        df_vol['quoteVolume'] = df_vol['quoteVolume'].astype(float)
        top_20 = df_vol.sort_values(by='quoteVolume', ascending=False).head(20)
        return [f"{s[:-4]}/USDT" for s in top_20['symbol']]
    except: return ["BTC/USDT", "ETH/USDT", "SOL/USDT", "FET/USDT"]

def calcular_rsi(precios, periodo=14):
    if len(precios) < periodo + 1: return 50
    deltas = np.diff(precios)
    ganancias = deltas.clip(min=0); perdidas = -deltas.clip(max=0)
    avg_gain = np.mean(ganancias[-periodo:]); avg_loss = np.mean(perdidas[-periodo:])
    return 100 - (100 / (1 + (avg_gain / (avg_loss if avg_loss != 0 else 0.001))))

# --- SESIÓN ---
if 'saldo_demo' not in st.session_state:
    st.session_state.update({
        'saldo_demo': 1000.0, 'ganancia_total': 0.0, 'posiciones': [], 
        'precios_hist': [], 'ordenes_malla': [], 'ultimo_par': "", 
        'historial_pnl': [], 'direccion': 'LONG', 'user_name': 'Gomez'
    })

# --- SIDEBAR (CON API Y MODOS RECUPERADOS) ---
with st.sidebar:
    st.image("https://upload.wikimedia.org/wikipedia/commons/thumb/1/1a/Sol_de_Mayo-Bandera_de_Argentina.svg/1200px-Sol_de_Mayo-Bandera_de_Argentina.svg.png", width=100)
    st.markdown("### ☀️ Configuración")
    
    par = st.selectbox("🎯 Objetivo (Binance):", obtener_top_20_binance())
    
    st.divider()
    entorno = st.radio("Entorno:", ["🟢 MODO DEMO", "🟡 MODO REAL"])
    api_key = st.text_input("API Key", type="password")
    secret_key = st.text_input("Secret Key", type="password")
    
    st.divider()
    lev = st.slider("Apalancamiento", 1, 50, 50)
    niveles = st.number_input("Niveles de Malla", 1, 30, 10)
    distancia = st.slider("Distancia Niveles (%)", 0.01, 1.0, 0.09) / 100
    inversion = st.number_input("Inversión (USDT)", 1.0, 10000.0, 10.0)
    tp_global = st.slider("Take Profit (%)", 0.01, 2.0, 0.15) / 100

    if st.button("🚨 BOTÓN DE PÁNICO", use_container_width=True):
        st.session_state.update({'posiciones': [], 'ordenes_malla': []})
        st.rerun()

# --- HEADER ---
c_h1, c_h2 = st.columns([4, 1])
c_h1.markdown(f"## ☀️ H y G Inovaciones - <span class='user-tag'>👤 {st.session_state.user_name}</span>", unsafe_allow_html=True)

# --- LÓGICA DE CAZA ---
bot_on = st.toggle("🚀 ACTIVAR ALGORITMO PREDADOR")

if bot_on:
    try:
        res = requests.get(f"https://min-api.cryptocompare.com/data/price?fsym={par.split('/')[0]}&tsyms=USD").json()
        p_act = float(res['USD'])
        st.session_state.precios_hist.append(p_act)
        if len(st.session_state.precios_hist) > 50: st.session_state.precios_hist.pop(0)
        
        rsi_val = calcular_rsi(st.session_state.precios_hist)
        ema = np.mean(st.session_state.precios_hist[-10:]) if len(st.session_state.precios_hist) >= 10 else p_act
        st.session_state.direccion = "LONG" if p_act >= ema else "SHORT"

        # 1. CREAR MALLA
        if not st.session_state.ordenes_malla:
            m_nivel = inversion / niveles
            for i in range(niveles):
                f = (1 - (i * distancia)) if st.session_state.direccion == "LONG" else (1 + (i * distancia))
                st.session_state.ordenes_malla.append({'id': i+1, 'precio': round(p_act * f, 4), 'monto': round(m_nivel, 2), 'estado': 'PENDIENTE'})

        # 2. EJECUTAR
        for o in st.session_state.ordenes_malla:
            if o['estado'] == 'PENDIENTE':
                hit = (st.session_state.direccion == "LONG" and p_act <= o['precio']) or \
                      (st.session_state.direccion == "SHORT" and p_act >= o['precio'])
                if hit and st.session_state.saldo_demo >= o['monto']:
                    st.session_state.saldo_demo -= o['monto']
                    o['estado'] = 'EJECUTADA'
                    st.session_state.posiciones.append({'entrada': p_act, 'monto': o['monto']})

        # 3. CIERRE
        if st.session_state.posiciones:
            t_inv = sum(p['monto'] for p in st.session_state.posiciones)
            p_prom = sum(p['entrada'] for p in st.session_state.posiciones) / len(st.session_state.posiciones)
            
            if st.session_state.direccion == "LONG":
                ganancia = (t_inv * (p_act / p_prom - 1)) * lev
                cierra = p_act >= p_prom * (1 + tp_global)
            else:
                ganancia = (t_inv * (1 - p_act / p_prom)) * lev
                cierra = p_act <= p_prom * (1 - tp_global)

            if cierra and ganancia > 0:
                st.session_state.historial_pnl.append({'Fecha': datetime.now().strftime("%H:%M:%S"), 'Par': par, 'Tipo': st.session_state.direccion, 'Ganancia': round(ganancia, 2)})
                st.session_state.saldo_demo += (t_inv + ganancia)
                st.session_state.ganancia_total += ganancia
                st.session_state.update({'posiciones': [], 'ordenes_malla': []})
                st.balloons(); st.rerun()

        # --- PANEL VISUAL ---
        c1, c2, c3 = st.columns(3)
        c1.metric(f"Precio ({st.session_state.direccion})", f"${p_act:,.4f}")
        c2.metric("Saldo Billetera", f"${st.session_state.saldo_demo:,.2f}")
        c3.metric("Cosecha Total", f"${st.session_state.ganancia_total:,.2f}", f"RSI: {rsi_val:.1f}")

        fig = go.Figure()
        fig.add_trace(go.Scatter(y=st.session_state.precios_hist, line=dict(color='#F0B90B', width=2), fill='tozeroy', name="Precio"))
        for o in st.session_state.ordenes_malla:
            col = "green" if o['estado'] == 'EJECUTADA' else "rgba(255,255,255,0.2)"
            fig.add_hline(y=o['precio'], line_dash="dash", line_color=col)
        fig.update_layout(height=300, template="plotly_dark", margin=dict(l=0,r=0,b=0,t=0))
        st.plotly_chart(fig, use_container_width=True)

        ca, cb = st.columns(2)
        with ca: st.markdown("##### 📋 Malla"); st.dataframe(st.session_state.ordenes_malla, use_container_width=True, height=200)
        with cb: st.markdown("##### 📜 Historial"); st.dataframe(st.session_state.historial_pnl[::-1], use_container_width=True, height=200)

        time.sleep(1); st.rerun()
    except: time.sleep(1); st.rerun()
        

...
if not st.session_state.autenticado:
    ...
    with st.sidebar:
        ...
        inversion = st.number_input("Inversión Total (USDT)", 1.0, 10000.0, 10.0)
        ...
        if st.button("🚨 BOTÓN DE PÁNICO", use_container_width=True):
            ...
            st.warning("Operaciones cerradas.")
            st.rerun()
    ...
    if bot_on:
        try:
            ...
            st.session_state.direccion = "LONG" if precio_act >= ema_rapida else "SHORT"
            ...
            if not st.session_state.ordenes_malla:
                ...
                for i in range(niveles):
                    ...
                    st.session_state.ordenes_malla.append({'id': i+1, 'precio': round(precio_act * factor, 4), 'monto': round(monto_n, 2), 'estado': 'PENDIENTE'})
            ...
            for o in st.session_state.ordenes_malla:
                if o['estado'] == 'PENDIENTE':
                    ...
                    if toco and st.session_state.saldo_demo >= o['monto']:
            ...
            c3.metric("Ganancia Total", f"${st.session_state.ganancia_total:,.2f}", f"RSI: {rsi_val:.1f}")
            ...
            with col_a: st.markdown("##### 📋 Malla"); st.dataframe(st.session_state.ordenes_malla, use_container_width=True, height=250)
            with col_b: st.markdown("##### 📜 Historial"); st.dataframe(st.session_state.historial_pnl[::-1], use_container_width=True, height=250)

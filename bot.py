...
if not st.session_state.autenticado:
    ...
    with st.sidebar:
        ...
        entorno = st.radio("Entorno de Operación:", ["🟢 MODO DEMO", "🟡 MODO REAL"])
        api_key = st.text_input("Binance API Key", type="password", placeholder="Introduce tu Key")
        secret_key = st.text_input("Binance Secret Key", type="password", placeholder="Introduce tu Secret")
        
        st.divider()
        lev = st.slider("Apalancamiento", 1, 50, 20)
        niveles = st.number_input("Niveles de Malla", 1, 30, 10)
        distancia = st.slider("Distancia Niveles (%)", 0.01, 2.0, 0.1) / 100

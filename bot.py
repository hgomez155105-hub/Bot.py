...
if 'ganancia_acumulada' not in st.session_state:
    st.session_state.update({
        ...
        'ultimo_par': "", # Para detectar cambios de moneda
        'ultimo_modo': ""  # Para detectar cambios demo/real
    })
...
with st.sidebar:
    ...
    st.markdown("---")
    par = st.selectbox("Activo:", ["SOL/USDT", "BTC/USDT", "ETH/USDT", "XRP/USDT"])
    
    # --- LÓGICA DE AUTO-RESET AL CAMBIAR MONEDA O MODO ---
    if par != st.session_state.ultimo_par or modo != st.session_state.ultimo_modo:
        ...
        # No reseteamos ganancia_acumulada para que el usuario vea su total diario
    
    leverage = st.slider("Apalancamiento (x)", 1, 50, 20)
    monto = st.number_input("Inversión (USDT)", value=5.0)
    dist_grid = st.slider("Profit Objetivo (%)", 0.05, 1.0, 0.1) / 100
    dist_recompra = 0.5 / 100 

    if st.button("🚨 RESET TOTAL DE SESIÓN", type="primary"):
        st.session_state.posiciones = []
...
# --- INTERFAZ PRINCIPAL ---
st.markdown(f"<h3 style='text-align: center; color: white;'>🚀 {modo} - {par}</h3>", unsafe_allow_html=True)
...
if bot_on:
    try:
        ...
        # Evitar saltos en el historial si la lista estaba vacía
        ...
        # 3. LÓGICA DE CIERRE (Scalping Rápido)
        for i, pos in enumerate(st.session_state.posiciones):
            ...
            if precio >= target:
                ...
                new_log = pd.DataFrame([{"Hora": datetime.now().strftime("%H:%M:%S"), "Evento": f"PROFIT {par}", "Precio": precio, "PNL": f"${pnl:.2f}"}])
        ...
        # --- GRÁFICO ---
        fig = go.Figure()
        fig.add_trace(go.Scatter(y=st.session_state.precios_hist, mode='lines+markers', line=dict(color='#00FF00', width=2), marker=dict(size=4)))
        
        for p in st.session_state.posiciones:
            fig.add_hline(y=p['precio'], line_color="white", line_width=1, annotation_text="ENTRY")
